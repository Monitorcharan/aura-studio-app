...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    .then(async (response) => {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Signature rejected. Verify credentials.');
      }
      return data;
    })
    .then((data) => {
  ...
  return (
    <>
      <Preloader title="AURA / SECURE" subtitle="ENCRYPTED GATEWAY" />
      ...
      <main className="min-h-screen flex items-center justify-center px-4 sm:px-8 md:px-24 py-24 sm:py-28 max-w-7xl mx-auto w-full relative">
        <div className="grid lg:grid-cols-12 items-center gap-8 sm:gap-12 w-full">
          ...
          <div className="hidden lg:flex lg:col-span-6 flex-col justify-between h-[45vh] text-left select-none pointer-events-none fade-in-up">
            <div className="text-accentCyan font-mono text-xs uppercase tracking-[0.3em]">Access Portal</div>
            <div>
              ...
              <p className="font-light max-w-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                Sign in to manage your signature grooming appointments, stylist notes, and reservation schedules.
              </p>
            </div>
            <div className="text-xs font-mono uppercase tracking-widest" style={{ color: 'var(--text-subtle)' }}>AURA / SECURITY DIVISION</div>
          </div>
          ...
          <div className="lg:col-span-6 flex justify-center lg:justify-end w-full fade-in-up">
            <div className="glass-panel rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 w-full max-w-md shadow-2xl relative" style={{ borderColor: 'var(--surface-border)' }}>
              <div className="text-center lg:text-left mb-8">
                ...
                <h2 className="text-2xl font-bold font-display" style={{ color: 'var(--heading-color)' }}>Identity Signature</h2>
                <p className="text-sm font-light mt-1" style={{ color: 'var(--text-muted)' }}>Authenticate your session to proceed.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="email" className="block text-xs font-mono uppercase tracking-widest mb-2" style={{ color: 'var(--text-muted)' }}>
                    Email Address
                  </label>
                  <input
                    type="email"
                    ...
                    style={{ backgroundColor: 'var(--input-bg)', borderWidth: '1px', borderColor: 'var(--input-border)', color: 'var(--text-color)' }}
                  />
                </div>
              ...
              <div className="mt-8 text-center text-xs font-mono" style={{ color: 'var(--text-subtle)' }}>
                New occupant?{' '}
                <Link to="/register" className="text-accentCyan font-bold hover:opacity-80 transition cursor-none">
                  Create account
                </Link>
              </div>
            </div>
